...
def clbk_laser(msg):
    global regions_
    regions_ = {
        'right':  min(min(msg.ranges[255:285]), 10),# right is at 270 deg
        'fright': min(min(msg.ranges[285:324]), 10),
        'fleft':  min(min(msg.ranges[20:50]), 10),
        'front':  min(min(msg.ranges[0:20]), min(msg.ranges[325:359])), # front is at 0 deg
        'left':   min(min(msg.ranges[50:120]), 10), # left is at 90 deg
    }
