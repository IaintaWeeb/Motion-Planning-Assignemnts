...
ob1_pos=np.array([0.50,1.57])
ob2_pos=np.array([0.61,-1.34])
ob3_pos=np.array([-1.26,-0.39])
ob4_pos=np.array([1.32,0.81])
q_star=[0.5,0.5,0.5,0.5]
# parameters
...
def heading():
    ...
    F_total=F_att()+F_rep(ob1_pos,q_star[0])+F_rep(ob2_pos,q_star[1])+F_rep(ob3_pos,q_star[2])+F_rep(ob4_pos,q_star[3]) 
...
def main():
    ...
    while not rospy.is_shutdown():
        print(F_att(),F_rep(ob1_pos,q_star[0]),F_rep(ob2_pos,q_star[1]),F_rep(ob3_pos,q_star[2]),F_rep(ob2_pos,q_star[3]),r_pos)
        if state_ == 0:
            heading()
        elif state_ == 1:
            move()
