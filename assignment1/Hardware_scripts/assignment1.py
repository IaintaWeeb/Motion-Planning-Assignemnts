...
def clbk_odom(msg):
    global position_, yaw_,pos_list
...
def clbk_laser(msg):
    ...
    regions_ = {
        'right':  min(min(laserdata[53:73]), 10),# right is at 270 deg
        'fright': min(min(laserdata[73:119]), 10),
        'fleft':  min(min(laserdata[129:170]), 10),
        'front':  min(min(laserdata[89:149]), 10), # front is at 0 deg
        'left':   min(min(laserdata[170:190]), 10), # left is at 90 deg
    }
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if state_ == 0:
            if regions_['front'] > 0.10 and regions_['front'] < 0.2:
                p_leave = position_
                p_hit = position_
                count_state_time_=0
                change_state(1)
